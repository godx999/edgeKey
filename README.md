...
> **注：** 
> 1. 点击按钮后，Cloudflare 会引导你登录并授权 Git 账户。它会自动将本仓库 fork 到你的名下。
> 2. 在向导中填写配置（包括 `AUTH_SECRET`）并绑定或新建 D1 数据库。
> 3. Cloudflare 默认执行 `npm run deploy` 脚本，它会自动完成数据库建表与初始种子数据（管理员账号等）的操作，无需手动干预。
> 4. 线上默认管理员账号为 `admin / admin123456`，首次登录后请务必在后台修改密码！
